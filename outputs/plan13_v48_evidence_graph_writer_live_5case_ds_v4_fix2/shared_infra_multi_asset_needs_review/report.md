...
- **本轮不覆盖的范围**：本轮不把未经过独立验证的候选对象 ws-legal-09、ws-legal-12、203.0.113.131、cdn-auth-queue.net、legal-docs-cdn.example、login-queue-cdn.net、socket.io、vercel.app、www.microsoft.com、www.opswat.com、www.reddit.com、www.safeaeon.com、www.stamus-networks.com 直接写成已确认范围，也不把背景情报直接写成已确认事实。
...
- **扩展查询候选**：ws-legal-09、ws-legal-12、203.0.113.131、cdn-auth-queue.net、legal-docs-cdn.example、login-queue-cdn.net、socket.io、vercel.app、www.microsoft.com、www.opswat.com、www.reddit.com、www.safeaeon.com、www.stamus-networks.com
- **种子资产**：ws-legal-03
...
- **对象覆盖边界**：当前调查重点仍围绕 `ws-legal-03` 收敛，但还没有资产可以写成已确认受影响范围；候选对象 ws-legal-09、ws-legal-12、203.0.113.131、cdn-auth-queue.net、legal-docs-cdn.example、login-queue-cdn.net、socket.io、vercel.app、www.microsoft.com、www.opswat.com、www.reddit.com、www.safeaeon.com、www.stamus-networks.com 继续作为边界线索保留。
...
- **当前不能确认的机制环节**：仍需围绕当前关键关联指标开展一轮扩线核查，确认是否存在同指标的更大范围复现。、是否存在主机侧执行、持久化或横向移动证据？、关联资产 ws-legal-09、ws-legal-12 是否真正受影响？
- **本节主要依据来源**：种子事件窗观测、关联事件扩查、显式反证检查
...
| 2026-06-20 10:42:00 UTC | 资产 `ws-legal-09`：存在共享基础设施背景，相关外部对象不能仅凭一次共现就升格为攻击基础设施 | 背景观测 |
...
| 2026-06-20 10:49:30 UTC | 资产 `ws-legal-12`：存在共享基础设施背景，相关外部对象不能仅凭一次共现就升格为攻击基础设施 | 背景观测 |
...
| 2026-06-20 10:55:00 UTC | 资产 `ws-legal-03`：同时间窗存在已批准的维护、变更或备份活动，可解释部分日常管理行为 | 反证检查 |
...
## 7. 传播与关联分析
- **关联扩查主轴**：auth-queue-sync.net、cdn-auth-queue.net、identity-cache.vendor.example、legal-docs-cdn.example、login.microsoftonline.com、login-queue-cdn.net、www.microsoft.com、www.reddit.com、www.opswat.com、www.stamus-networks.com、www.safeaeon.com、socket.io、vercel.app、198.51.100.131、20.190.160.22、203.0.113.131、t13d190900_7711aa22bb33_9911ccddee77
...
- **候选关联**：仍需围绕当前关键关联指标开展一轮扩线核查，确认是否存在同指标的更大范围复现。、是否存在主机侧执行、持久化或横向移动证据？、关联资产 ws-legal-09、ws-legal-12 是否真正受影响？
- **未完成独立验证的对象**：ws-legal-09、ws-legal-12、203.0.113.131、cdn-auth-queue.net、legal-docs-cdn.example、login-queue-cdn.net、socket.io、vercel.app、www.microsoft.com、www.opswat.com、www.reddit.com、www.safeaeon.com、www.stamus-networks.com
...
- **当前攻击阶段**：命令与控制
- **仍未确认的影响**：当前仍缺少对“仍需围绕当前关键关联指标开展一轮扩线核查，确认是否存在同指标的更大范围复现。”的直接补证。、当前尚未观测到稳定的执行阶段证据，因此还不能确认主机侧执行、持久化或横向移动是否已经成立。、当前仍缺少对“关联资产 ws-legal-09、ws-legal-12 是否真正受影响？”的直接补证。
- **本节主要依据来源**：显式反证检查、关联事件扩查
...
- **因缺口而不能写出的结论**：当前仍缺少对“仍需围绕当前关键关联指标开展一轮扩线核查，确认是否存在同指标的更大范围复现。”的直接补证。、当前尚未观测到稳定的执行阶段证据，因此还不能确认主机侧执行、持久化或横向移动是否已经成立。、当前仍缺少对“关联资产 ws-legal-09、ws-legal-12 是否真正受影响？”的直接补证。
- **下一轮最值得补的证据**：存在理论上相关的工具，但它们近期已经进入低收益状态。
- **本节主要依据来源**：种子事件窗观测、关联事件扩查、显式反证检查
...
- **为什么当前结论成立**：当前已经形成：种子告警已经和明确异常起点对齐；同一基础设施的连续通信已在同资产或关联资产上被复现；异常范围已经不再局限于种子资产；已核查的维护或更新背景不足以解释整条主链；但由于仍需围绕当前关键关联指标开展一轮扩线核查，确认是否存在同指标的更大范围复现，本案仍需要按待人工复核结论交付。
...
### 10.3 持续监控或复核动作
- 当前暂无需要单列的持续监控或复核动作。
- **下一次复核最值得补的证据**：存在理论上相关的工具，但它们近期已经进入低收益状态。
