...
- 反证与替代解释：检索到 1 条更接近维护、更新或正常基线的事件。；显式反证检查发现 2 条更接近维护、更新、补丁或备份背景的事件。
...
- `fact-counter-cl-04` [替代解释]：显式反证检查发现 2 条更接近维护、更新、补丁或备份背景的事件。；书写边界：要解释为何不足以推翻主判断，而不是只罗列背景事件
### 缺口事实
...
- 优先引用事实 ID（按顺序）：fact-gap-ground_external_context；fact-counter-cl-03；fact-counter-cl-04；fact-event-evt-611
### 10. 结论与后续建议
