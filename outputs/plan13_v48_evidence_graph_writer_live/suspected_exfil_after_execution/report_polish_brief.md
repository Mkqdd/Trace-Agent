...
- `fact-scope-obj-15` [待确认对象]：对象 `attack.mitre.org` 当前作为候选扩展对象保留为待确认范围。；书写边界：仍需独立验证，不能直接升格
- `fact-scope-obj-20` [待确认对象]：对象 `t13d190900_44ac2200b3c1_ab11cd22ee44` 当前作为候选扩展对象保留为待确认范围。；书写边界：仍需独立验证，不能直接升格
...
- 优先引用事实 ID（按顺序）：fact-scope-obj-01；fact-scope-obj-03；fact-scope-obj-04；fact-scope-obj-02；fact-counter-cl-05
### 3. 对象覆盖策略与关键实体
...
- 优先引用事实 ID（按顺序）：fact-event-evt-602；fact-event-evt-604；fact-counter-cl-05
### 6. 时序特征与行为模式
...
- 优先引用事实 ID（按顺序）：fact-counter-cl-05；fact-counter-cl-10；fact-event-evt-611
### 10. 结论与后续建议
- 本节目标：按立即处置、短期核查、持续复核三类写动作建议，并回扣前文证据边界。
- 可引用 packets：verdict_packet；action_packet；constraint_packet
- 优先引用事实 ID（按顺序）：fact-scope-obj-01；fact-scope-obj-03；fact-scope-obj-04；fact-action-01；fact-action-02；fact-action-03
### 11. 技术附录提示
- 本节目标：只提示附录里有哪些技术明细可以进一步查阅，不重复附录内容。
- 可引用 packets：scope_packet；constraint_packet
- 优先引用事实 ID（按顺序）：fact-counter-cl-05
