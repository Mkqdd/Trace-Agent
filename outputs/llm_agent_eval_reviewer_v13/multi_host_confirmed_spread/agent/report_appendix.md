...
| 时间 | 事件 | 作用 | 观测引用 |
| -- | -- | -- | -- |
| 2026-05-18 08:55:00 UTC | The seed workstation resolved a rare domain later reused by the beacon alert. | `主证据` | obs-001, obs-002, obs-005 |
| 2026-05-18 09:00:00 UTC | Seed JA4 alert matched a beacon against the same rare external infrastructure. | `种子告警` | obs-001, obs-002, obs-005 |
| 2026-05-18 09:06:20 UTC | The seed workstation reconnected to the same C2 infrastructure after the alert. | `主证据` | obs-001, obs-002, obs-005 |
| 2026-05-18 09:11:30 UTC | PsExec remote service creation from ws-eng-02 to ws-eng-05 was observed after beaconing started. | `主证据` | obs-001, obs-002, obs-005 |
...
- **观测引用 / 来源引用**：`obs-003, obs-004, obs-005`
...
| `obs-002` | `search_related_events` | `trace_store.related_events` | 扩线检索到 7 条可疑关联事件，围绕 sync-cdn-notify.net、203.0.113.77、10.70.3.45 展开；当前先作为待验证候选线索。 |
| `obs-003` | `ground_candidate_event` | `candidate_grounding` | 候选事件 evt-406 缺少可独立验证的指示物，当前不并入主证据链，仅保留为边界线索。 |
| `obs-004` | `ground_candidate_event` | `candidate_grounding` | 候选事件 evt-407 已完成 local_intel、abuse_ch grounding，但暂未命中独立情报，当前不直接并入主证据链。 |
| `obs-005` | `check_counterevidence` | `trace_store.asset_context` | 显式反证检查覆盖了 7 条同资产上下文，未发现足以降级当前判断的明确背景解释。 |
