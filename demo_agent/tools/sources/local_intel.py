...
_TYPE_CANDIDATES = {
    ...
    "JA4": ["ja4"],
    "SSL_SHA1": ["ssl_sha1"],
    "CERT_SHA1": ["cert_sha1", "ssl_sha1"],
    "MD5": ["md5"],
    "SHA1": ["sha1", "ssl_sha1", "cert_sha1"],
    "SHA256": ["sha256"],
}
...
class LocalIntelClient:
    ...
    def enabled(self) -> bool:
        return pymysql is not None and bool(
            self.db_config.host and self.db_config.name and self.db_config.user
        )

    def lookup(self, *, indicator_type: str, indicator_value: str) -> Dict[str, Any]:
        indicator_value = str(indicator_value or "").strip()
        indicator_type = str(indicator_type or "").strip().upper()
